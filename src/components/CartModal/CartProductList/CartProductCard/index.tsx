...
export const CartProductCard = ({ currentOffer }: IOfferCartProps) => {
  ...
  const newPrice: number = currentOffer.originalPrice - (currentOffer.discount / 100) * currentOffer.originalPrice
  ...
  return (
    <StyledCartProductCard>
      ...
      <small>{currentOffer.originalPrice.toLocaleString("pt-BR", {
              style: "currency",
              currency: "BRL",
            })}</small>
      <img onClick={() => removeItemFromCart(currentOffer.id)} src={trash} alt="Lixo" />
    </StyledCartProductCard>
  );
};
