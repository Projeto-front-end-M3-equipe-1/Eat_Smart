...
export const Header = ({ setIsEditProfileModalOpen }: IOpenModal) => {
  ...
  const foodCategory = localStorage.getItem("@EatSmart:userCommerceFoodCategory")?.toUpperCase();
  ...
  const userName =
    userState?.userName && userState?.userName[0].toUpperCase() + userState?.userName.substring(1);
        ...
        <li>
          <div>
            ...
            <nav>
              ...
              <button type="button" onClick={() => setIsEditProfileModalOpen(true)}>
                <i className="fa-solid fa-gears"></i>
              </button>
            </nav>
          </div>
